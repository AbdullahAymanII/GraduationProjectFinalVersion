...
def predict():
    ...
    try:
        ...
        multi['Binary_Predictions'] = [
            "Benign" if pred == 1 else "Attack" for pred in binary_predictions
        ]
        multiLabels1 = multiLabels.get("MultiLabel", {})  # Get the "MultiLabel" dictionary
        multiLabels2 = {v: k for k, v in multiLabels1.items()}
        ...
        for idx, pred in zip(attack_indices, multi_predictions):
            multi.at[idx, 'Multi_Predictions'] = multiLabels2.get(pred, f"Unknown({pred}")
